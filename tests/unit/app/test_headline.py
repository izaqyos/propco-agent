"""Headline metric extraction: which numbers get the big st.metric callout."""
...
def test_period_compare_headline_shows_both_periods(ledger: pd.DataFrame) -> None:
    result = compare_periods(
        ledger, Period.quarter(2025, 1), Period.quarter(2024, 1), LedgerFilter(), **KW
    )
    assert headline_metric([result]) == [
        ("2025-Q1", "€361,810.32", "+37.93%"),
        ("2024-Q1 (baseline)", "€262,309.07", None),
    ]
