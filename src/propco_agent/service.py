...
class AssetManagerService:
    ...
    def ask(self, question: str, *, thread_id: str, policy: DataPolicy | None = None) -> AskResult:
        ...
        out = self.graph.invoke(self._state(question, thread_id, policy), self._config(thread_id))
        return self._result(out, thread_id, started)
    ...
    def stream(
        ...
        yield self._result(self.graph.get_state(config).values, thread_id, started)
